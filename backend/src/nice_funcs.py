...
def token_overview(address):
    ...
    """
    BIRDEYE_API_KEY = get_bird_eye_key()
    ...
    if response.status_code == 200:
        ...
        # Extract andcess description links if extensions are not None
...
def market_buy(token, amount, slippage, logger=None):
    import base64
    import json
    ...
    logger(f"https://solscan.io/tx/{str(txId)}")


def market_sell(QUOTE_TOKEN, amount, slippage, logger=None):
    import base64
    import json
...
def get_data(address, days_back_4_data, timeframe, logger=None):
    BIRDEYE_API_KEY = get_bird_eye_key()
    if not logger:
    ...
    if os.path.exists(temp_file):
        logger(f"cached data for {address[:4]}")
    ...
    if response.status_code == 200:
        ...
        if len(df) < 40:
            logger(
                "Padding data to ensure minimum 40 rows for analysis! 🚀"
            )
        ...
        logger(f"Ready! Processing {len(df)} candles... 🎯")
        ...
        logger(
            f"❌ Error: Failed to fetch data for address {address}. Status code: {response.status_code}"
        )
        if response.status_code == 401:
            logger("🔑 Check your BIRDEYE_API_KEY in .env file!")
        return pd.DataFrame()


def fetch_wallet_holdings_og(address, logger=None):
    BIRDEYE_API_KEY = get_bird_eye_key()
    API_KEY = (
    ...
    if response.status_code == 200:
        ...
        if (
            ...
            and "items" in json_response["data"]
            and json_response["data"]["items"]
        ):
            ...
            logger("No data available in the response.", "white")
    else:
        logger(f"Failed to retrieve token list for {address}.", "white")

    # Print the DataFrame if it's not empty
    if not df.empty:
        logger(df)
        # Assuming cprint is a function you have for printing in color
        logger(f"** Total USD balance is {df['USD Value'].sum()}", "white", "on_green")
        ...
        logger("No wallet holdings to display.", "white", "on_red")
    return df


def fetch_wallet_token_single(address, token_mint_address, logger=None):
    df = fetch_wallet_holdings_og(address, logger=logger)
...
def chunk_kill(token_mint_address, max_usd_order_size, slippage, logger=None):
    ...
    logger("AI Agent initiating position exit...", "white", "on_cyan")

    try:
        # Get current position using address from config
        address = os.getenv("WALLET_ADDRESS")
        df = fetch_wallet_token_single(address, token_mint_address, logger=logger)
        if df.empty:
            logger("❌ No position found to exit", "white", "on_red")
            return
        ...
        while (
            ...
            for i in range(3):
                try:
                    ...
                    market_sell(token_mint_address, sell_size, slippage, logger=logger)
                    logger(f"✅ Sell chunk {i + 1}/3 complete", "white", "on_green")
                    time.sleep(2)  # Small delay between chunks
                except Exception as e:
                    logger(f"❌ Error in sell chunk: {str(e)}", "white", "on_red")
        ...
        logger(f"❌ Error during position exit: {str(e)}", "white", "on_red")
...
def close_all_positions():
    ...
    dont_trade_list = DO_NOT_TRADE_LIST
    ...
    while pos_usd < (0.97 * usd_size) and (price < buy_under):
        ...
        try:
            for i in range(orders_per_open):
                ...
                cprint(
                    f"chunk buy submitted of {symbol[:4]} sz: {chunk_size}",
                    "white",
                    "on_blue",
                )
                time.sleep(1)
...
def ai_entry(symbol, amount, logger=None):

    if not logger:

        def logger(msg, ignore=None):
            print(msg)

    logger(
        "🤖 Agent initiating position entry...",
        "white",
        "on_blue",
    )
    ...
    logger(f"📊 Current position: ${pos_usd:.2f} USD", "white", "on_blue")

    # Check if we're already at or above target
    if pos_usd >= (target_size * 0.97):
        logger("✋ Position already at or above target size!", "white", "on_blue")
        return

    # Calculate how much more we need to buy
    size_needed = target_size - pos_usd
    if size_needed <= 0:
        logger("🛑 No additional size needed", "white", "on_blue")
        return
    ...
    while pos_usd < (target_size * 0.97):
        logger(f"🤖 AI Agent executing entry for {symbol[:8]}...", "white", "on_blue")
        logger(
        ...
        try:
            for i in range(orders_per_open):
                market_buy(symbol, chunk_size, slippage, logger=logger)
                logger(
            ...
            try:
                logger(f"Error encountered by buying: {str(e)}")
                logger(
    ...
    logger("✨ AI Agent completed position entry", "white", "on_blue")


def get_token_balance_usd(token_mint_address, logger=None):
    """Get the USD value of a token position for wallet """
    if not logger:
    ...
    try:
        ...
        df = fetch_wallet_token_single(
            address, token_mint_address, logger=logger
        )  # Using address from config
        if df.empty:
            logger(f"🔍 No position found for {token_mint_address[:8]}")
            return 0.0
        ...
        logger(f"❌ Error getting token balance: {str(e)}")
        return 0.0


# if  __name__ == "__main__" :
#     hold = fetch_wallet_holdings_og(address)
#     print(f"{hold=}")
